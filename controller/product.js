...
const updateProduct = asyncHandler(async (req, res, next) => {
  ...
  // Check if productImage is provided in the request
  if (!file) {
  ...
  if (file.size > process.env.FILE_UPLOAD_SIZE) {
    throw createError(
      400,
      `Please upload an image of size less than ${process.env.FILE_UPLOAD_SIZE}`
    );
  }
  ...
  cloudinary.uploader.upload(
    ...
    async function (error, result) {
      if (error) {
        throw createError(409, `Failed to update product`);
      }

      const updatedProduct = await Product.findByIdAndUpdate(
        req.params.productId,
        { ...req.body, productImage: result.url },
        {
          new: true,
          runValidators: true,
        }
